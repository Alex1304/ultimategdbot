...
- `run()` returns `Publisher<?>`, starts with
  `ctx.event().deferReply().then(grammar.resolve(ctx.event())).flatMap(options -> ...)`
...
- All user-facing strings via `ctx.translate(Strings.GD, "key")` or `ctx.translate(Strings.GENERAL, "key")` — add
  placeholder keys to the properties files

**Command with subcommands:**

- Outer class: `public final class FooCommand` with `@ChatInputCommand(name=..., subcommands={...})` — NO `@RdiService`
  on the outer class
- Each subcommand: `@RdiService public static final class SubName implements ChatInputInteractionListener` with its own
  `@RdiFactory` constructor
