...
//				+ "\n```\n" + "See `" + ctx.getPrefixUsed() + "help " + ctx.getArgs().get(0) + "` for more information.").then());
		addHandler(PermissionDeniedException.class, (e, ctx) -> ctx.reply(ctx.translate("strings.common", "command_perm_denied")).then());
		addHandler(ClientException.class, (e, ctx) -> {
			...
			var message = responseOptional.map(response -> {
				...
				return ctx.translate("strings.common", "command_discord_error") + "\n"
						+ Markdown.code(e.getStatus().code() + " " + e.getStatus().reasonPhrase()) + "\n"
						+ Markdown.codeBlock(sb.toString())
						+ (e.getStatus().code() == 403 ? ctx.translate("strings.common", "command_ensure_perms") : "");
			}).orElse(ctx.translate("strings.common", "command_discord_error"));
			
			return ctx.reply(message).then();
		});
		addHandler(DatabaseException.class, (e, ctx) -> Mono.when(
				ctx.reply(ctx.translate("strings.common", "command_database_access_error")),
